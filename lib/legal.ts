// In-app legal copy. Edit the wording here (fill every [PLACEHOLDER]) and the
// /terms, /privacy and /disclaimer pages update automatically.
// NOTE: starter drafts — have a solicitor review the Terms (esp. liability)
// before relying on them. Not legal advice.

export const termsMarkdown = `## Terms of Service

**Effective date:** [EFFECTIVE DATE]

These Terms govern your use of Swardly, a nutrient-management and farm record-keeping service ("**Service**") provided by [PROVIDER] ("**we**", "**us**", "**our**"). By creating an account or using the Service you agree to these Terms. If you do not agree, do not use the Service.

### 1. Who we are
Swardly is operated by [PROVIDER], [POSTAL ADDRESS]. Contact: [CONTACT EMAIL].

### 2. Eligibility and business use
The Service is intended for farmers and agricultural businesses using it in the course of their business. By using it you confirm that you are at least 18 years old and are acting for purposes relating to your trade, business, craft or profession, and **not as a consumer**. You confirm you have authority to agree to these Terms on behalf of the farm business you represent.

### 3. What the Service is — and is not
The Service lets you record field, crop, soil and application data and presents nutrient figures, plans, rates, schedules and similar outputs ("**Guidance**") generated from reference data (including AHDB RB209 and SRUC technical notes) and the information **you** enter.

Guidance is **decision-support and record-keeping information only**. It is **not** professional agronomic, environmental, veterinary, financial or legal advice, and is **not a substitute** for advice from a suitably qualified, FACTS-registered adviser who can assess your specific circumstances. See Section 9 and the Disclaimer.

### 4. Accounts and farm members
You must provide accurate registration details and keep them up to date. A farm account may have an **administrator** and additional users ("**Members**"). The administrator is responsible for managing Members, for all activity under the account, and is the controller of the farm data held in the account. You are responsible for keeping login credentials secure and must tell us promptly at [CONTACT EMAIL] of any suspected unauthorised use.

### 5. Subscriptions, fees and payment
5.1 Access to paid features requires a subscription. The fees, billing frequency and what is included are shown at sign-up or at [PRICING PAGE/URL] ("**Fees**").

5.2 Payments are handled by our payment provider, [PAYMENT PROCESSOR]. We do not store your full card details.

5.3 Unless stated otherwise, subscriptions **renew automatically** each [BILLING PERIOD] until cancelled. You may cancel at any time, effective at the end of the current paid period.

5.4 Except where the law requires otherwise, Fees are **non-refundable**, including for unused time in a billing period.

5.5 We may change Fees on at least [NOTICE DAYS] days' notice; changes take effect from your next renewal. If you do not accept a change, you may cancel before it takes effect.

5.6 Fees are [VAT STATUS]. You are responsible for any taxes that apply to you.

5.7 If payment fails or is overdue, we may suspend or withdraw access to paid features.

### 6. Acceptable use
You must not: use the Service unlawfully; copy, resell, sublicense or commercially exploit the Service except as expressly permitted; reverse engineer or attempt to extract source code (except to the extent the law permits and we cannot prohibit); upload malicious code or interfere with the Service's operation or security; attempt to access data or accounts that are not yours; or use the Service as the **sole** basis for any decision without your own verification and professional input.

### 7. Your data and intellectual property
7.1 You keep all rights in the data you enter ("**Your Data**"). You grant us a non-exclusive licence to host, store, process and use Your Data to provide, maintain, secure and improve the Service and as described in our Privacy Policy.

7.2 We and our licensors own all rights in the Service, including its software, design and content (excluding Your Data). RB209 and other third-party reference materials remain the property of their respective owners.

7.3 You can export Your Data and delete your account at any time from within the Service. On deletion we will remove Your Data as described in the Privacy Policy, subject to limited retention required by law or held in routine backups for a short period.

### 8. Availability, changes and beta features
We aim to keep the Service available but do **not** guarantee that it will be uninterrupted, error-free or free from data loss. We may change, suspend or withdraw all or part of the Service, including for maintenance, updates, or technical or security reasons. Some features may be offered on a **beta** basis and may change or be withdrawn without notice. We recommend you keep your own records of critical information.

### 9. Disclaimers
9.1 The Service and all Guidance are provided **"as is"** and **"as available"**. To the maximum extent permitted by law, we exclude all warranties, conditions and representations not expressly set out in these Terms, including any implied warranties of satisfactory quality, fitness for a particular purpose, accuracy, and non-infringement.

9.2 Guidance depends entirely on the accuracy and completeness of the data you enter and any laboratory results you provide. We do not verify your inputs and give no warranty that Guidance is accurate, complete, current or suitable for your circumstances.

9.3 You are solely responsible for: checking all Guidance and inputs; your application, cropping and management decisions; and complying with all applicable laws and standards, including (without limitation) Nitrate Vulnerable Zone (NVZ) rules, the Farming Rules for Water, and any farm-assurance scheme requirements such as Red Tractor. Always cross-check Guidance against the current published RB209 and your own qualified adviser before acting.

### 10. Limitation of liability
10.1 Nothing in these Terms excludes or limits our liability for: (a) death or personal injury caused by our negligence; (b) fraud or fraudulent misrepresentation; or (c) any other liability that cannot lawfully be excluded or limited.

10.2 Subject to clause 10.1, we are **not liable** to you, whether in contract, tort (including negligence), breach of statutory duty or otherwise, for any: loss of profit, revenue, crops, yield, livestock, production or anticipated savings; loss of business, contracts, goodwill or reputation; loss or corruption of data; regulatory penalties, fines or remediation costs; or any indirect, special or consequential loss, in each case however arising.

10.3 Subject to clause 10.1, our **total aggregate liability** arising out of or in connection with the Service and these Terms is limited to the **greater of** (i) [LIABILITY CAP] and (ii) the total Fees you paid to us in the 12 months immediately before the event giving rise to the claim.

10.4 You acknowledge that the Service provides decision-support information to a professional/business user who remains responsible for verifying outputs and for their own decisions, and that the allocation of risk in these Terms is reasonable given the nature of the Service and the Fees charged.

### 11. Indemnity
You will indemnify us against losses, costs and claims we suffer arising from your breach of these Terms, your misuse of the Service, or your breach of any law or third-party right.

### 12. Termination
12.1 You may stop using the Service and close your account at any time.

12.2 We may suspend or terminate your access if you materially breach these Terms (including non-payment) or where we reasonably need to for legal, security or operational reasons. Where practical we will give notice and an opportunity to put things right.

12.3 On termination your right to use the Service ends. You will have [EXPORT WINDOW] to export Your Data, after which we may delete it (subject to backups and legal retention).

### 13. Changes to these Terms
We may update these Terms from time to time. We will post the updated version with a new effective date and, for material changes, give reasonable notice. Continuing to use the Service after changes take effect means you accept the updated Terms.

### 14. General
14.1 **Governing law and jurisdiction.** These Terms and any dispute arising out of them are governed by the law of England and Wales, and the courts of England and Wales have exclusive jurisdiction.

14.2 **Severability.** If any provision is found unenforceable, the rest remain in force.

14.3 **No waiver.** A delay in enforcing these Terms is not a waiver.

14.4 **Assignment.** We may assign these Terms in connection with a reorganisation or sale of the business. You may not assign your rights without our consent.

14.5 **Entire agreement.** These Terms, the Disclaimer and the Privacy Policy are the entire agreement between us regarding the Service.

14.6 **Contact.** Questions about these Terms: [CONTACT EMAIL].

### Sharing job sheets with third parties
The Service lets you send job sheets to other account holders (e.g. contractors) and create share links usable without an account. **You are responsible** for who you send jobs and links to. Information you include in a job (field names, boundaries, products, rates, notes) will be visible to those recipients. Records created from a third party's submission are only written after you approve them, and you are responsible for checking submissions before approval.

`;

export const disclaimerMarkdown = `## Disclaimer

**Last updated:** [EFFECTIVE DATE]

Please read this before relying on anything Swardly shows you.

**Swardly is a decision-support and record-keeping tool — not an agronomist.** The nutrient figures, recommendations, plans, rates and schedules it produces ("Guidance") are generated from published reference data (including AHDB RB209 and SRUC technical notes) and **the information you enter**. They are provided for general information only.

**Guidance is not professional advice.** It is not a substitute for advice from a suitably qualified, FACTS-registered agronomist (or a vet, environmental or other professional) who can assess your specific situation. Do not rely on it as your only basis for any decision.

**You are responsible for your decisions and your compliance.** That includes:

- checking every figure and input before you act on it;
- your application, cropping and management decisions; and
- complying with all applicable rules — including NVZ requirements, the Farming Rules for Water, and farm-assurance schemes such as Red Tractor.

**Accuracy depends on your data.** Guidance is only as good as the field, soil, lab and application information you enter. We do not check your inputs and we do not warrant that Guidance is accurate, complete, current or suitable for your circumstances. **Always cross-check against the current published RB209 and your own adviser before applying anything.**

**Liability.** To the maximum extent permitted by law, we accept no liability for any loss — including crop, yield, livestock, financial, environmental or regulatory loss — arising from your use of, or reliance on, the Service or any Guidance. Your use of the Service is governed by our Terms of Service, including the limitation of liability in Section 10.

If you do not accept this, please do not rely on the Service for decisions.

### AI assistant
Answers from the in-app assistant are generated by an AI model from your data and the app's reference material. They can be incomplete or wrong. Verify any figure, rate or regulatory point before acting on it; the assistant is not a substitute for a FACTS-qualified adviser.

### Job sheets and third-party records
Records created from job sheets reflect what the operator or contractor reported. Check submissions before approving them — approved entries are written into your farm records as-is.

`;

export const privacyMarkdown = `## Privacy Policy

**Effective date:** [EFFECTIVE DATE]

This policy explains how [PROVIDER] ("**we**") collects and uses personal data when you use Swardly. We are the **data controller** for that data.

**Contact:** [PRIVACY EMAIL] — [PROVIDER], [POSTAL ADDRESS]. ICO registration number: [ICO REG NO].

### 1. The data we hold
- **Account data** — your name, email address, password (stored securely), and your role on a farm account.
- **Farm and field data** — fields, areas, locations/boundaries, crops, soil results, applications, plans and similar records you enter.
- **Location data** — if you use map or "find my field" features, your device location at that time.
- **Documents** — soil or other reports you upload for data extraction.
- **Technical and usage data** — IP address, device/browser information, log data and how you use the Service.

### 2. Why we use it, and our lawful basis
- To **provide the Service** you've signed up for, including storing your records and generating Guidance — *basis: performance of a contract*.
- To **operate, secure, troubleshoot and improve** the Service — *basis: our legitimate interests*.
- To **take payment** and manage your subscription — *basis: contract / legal obligation*.
- To **send service messages** (account, security, billing) — *basis: contract / legitimate interests*.
- To send **optional updates or marketing**, if you ask for them — *basis: consent*, which you can withdraw at any time.
- To **comply with the law** and handle disputes — *basis: legal obligation / legitimate interests*.

### 3. Who we share it with (our processors)
We use trusted providers to run the Service. They process data on our instructions:

- **Supabase** — database, authentication and file storage. Your data is hosted in the [SUPABASE REGION] region.
- **Vercel** — application hosting and content delivery (servers may be outside the UK).
- **Anthropic** — powers AI features, including the in-app assistant and automatic extraction of data from soil reports you upload. Relevant data (which may include the contents of uploaded reports and your queries) is sent to Anthropic's API. Data sent via the API is **not used to train AI models**.
- **[PAYMENT PROCESSOR]** — processes subscription payments. We do not receive or store your full card details.
- **Map providers (Mapbox / Esri)** — serve map imagery; your IP address and the map area you view are shared to deliver tiles.

We do not sell your personal data, and we do not use advertising or third-party tracking.

### 4. International transfers
Some providers (for example Vercel and Anthropic) are based outside the UK. Where personal data is transferred outside the UK, we rely on an appropriate safeguard recognised under UK data protection law (such as the UK International Data Transfer Agreement/Addendum, Standard Contractual Clauses, or an adequacy/"data bridge" decision).

### 5. How long we keep it
We keep your personal data while your account is active. When you delete your account (which you can do at any time from within the Service), we delete your data from our live systems; it may persist for a short period in routine encrypted backups before being overwritten, and we may retain limited information where the law requires.

### 6. Your rights
You have the right to: access your data; correct it; delete it; restrict or object to processing; and receive a copy in a portable format. The Service includes **export** and **account-deletion** tools that let you do the last two yourself. To exercise any right, email [PRIVACY EMAIL].

### 7. Complaints
If you have a data-protection concern, please email [PRIVACY EMAIL]. We will acknowledge within [NOTICE DAYS] working days and aim to resolve it promptly. If you remain unsatisfied, you can complain to the Information Commissioner's Office (ICO) at **ico.org.uk** or **0303 123 1113**.

### 8. Cookies and on-device storage
We use a small number of **strictly necessary** cookies to keep you signed in and operate the Service. We also store **functional preferences** on your device — for example your plan settings, logging preferences and chosen units — so the app behaves the way you left it. We do **not** use advertising or third-party tracking technologies. Under current UK rules these essential and functional technologies do not require your consent, but you can clear them via your browser at any time (some features may not work as well if you do).

### 9. Security
We protect your data with measures including encryption in transit (HTTPS), encryption at rest, row-level access controls so users only see data they're entitled to, restricted administrative access, and optional two-factor authentication on your account. No system is completely secure, but we take reasonable steps to protect your information.

### 10. Data breaches
If a breach occurs that is likely to risk your rights, we will notify the ICO within 72 hours where required, and will inform you without undue delay if the risk to you is high.

### 11. Children
The Service is for business users aged 18 and over and is not directed at children.

### 12. Changes to this policy
We may update this policy. We will post the updated version with a new effective date and, for material changes, give reasonable notice.

### AI assistant ("Ask Swardly")
When you use the in-app assistant, your messages and relevant farm data needed to answer them are sent to **Anthropic** (our AI provider) for processing, and the assistant's tool lookups read your farm records to compose replies. We keep a **log of assistant questions and answers** (with the model used, tools called and timing) to improve the Service and monitor quality. Don't include information in assistant messages that you wouldn't want stored. Logs are retained for up to [LOG RETENTION, e.g. 12 months] and are deleted with your account.

### Notifications (web push)
If you turn on job alerts, your browser creates a push subscription (an endpoint and keys) which we store so we can send you notifications about job-sheet events. Turning alerts off, or revoking notification permission in your browser, removes the subscription.

### Location
The "locate me" button on maps uses your device's location **entirely on your device** to show your position relative to fields. Your position is **not sent to or stored on our servers**. Location is off by default and only activates when you tap the button and grant your browser's permission prompt.

### Job sheets shared outside your account
If you create a **share link** for a job sheet, anyone with that link (and PIN, if set) can view that job's details — field names, boundaries, products and rates — and submit completion information, without an account. You control who receives links and can revoke them at any time. If you send a job to a **contractor account**, that contractor (and any operator they forward it to) can see that job's details and, where they record it, time worked. They cannot see your other farm data.

`;

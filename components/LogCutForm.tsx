...
import { CutType, Field, Settings, YieldClass } from '@/lib/types';
import {
  CUT_TYPE_LABELS, fmt, getOfftakeForCut, getPlannedCuts, YIELD_CLASS_LABELS,
} from '@/lib/rules';
import { saveCut } from '@/lib/actions';

export function LogCutForm({
  field, settings, nextCutNumber, plannedType,
}: {
  field: Field;
  settings: Settings;
  ...
  const [cutType, setCutType] = useState<CutType>(plannedType);
  const [yieldClass, setYieldClass] = useState<YieldClass>('average');
  const [notes, setNotes] = useState('');

  const offtake = getOfftakeForCut(field.cut_profile, nextCutNumber, yieldClass, settings, cutType);
  const baseOfftake = getOfftakeForCut(field.cut_profile, nextCutNumber, 'average', settings, cutType);

  return (
    <form action={saveCut} style={{ paddingBottom: 100 }}>
      <input type="hidden" name="field_id" value={field.id} />
      <input type="hidden" name="cut_number" value={nextCutNumber} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 13, color: 'var(--amber)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Logging cut {nextCutNumber} of {field.cut_profile}
              </div>
              <div style={{ fontSize: 13, color: 'var(--ink-soft)' }}>This resets the "since last cut" tracker</div>
            </div>
          </div>
        </div>

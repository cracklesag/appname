...
export function SharedJobView({ token }: { token: string }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (isOfflineError(err)) {
        await enqueue('shared_job', { args: { token, pin: pin ?? '', completionsJson: JSON.stringify(completions) } });
        setPhase('queued');
        return;
      }
  ...
  if (phase === 'queued') {
    return (
      <div style={wrap}>
        <div style={{ ...card, textAlign: 'center' }}>
          <CheckCircle2 size={26} style={{ color: '#9a6320' }} />
          <div style={{ fontWeight: 700, margin: '8px 0 4px' }}>Saved on this phone</div>
          <div style={{ fontSize: 13.5, color: 'var(--muted)' }}>No signal just now — keep this page open (or come back to it) and it will send itself to the farm when you're back in coverage.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={wrap}>
      ...
      {(job?.business_name || job?.farm_name) && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>From</div>
          {job?.business_name && <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--forest, #2f7d6a)' }}>{job.business_name}</div>}
          {job?.farm_name && <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink-soft, #555)' }}>{job.farm_name}</div>}
        </div>
      )}

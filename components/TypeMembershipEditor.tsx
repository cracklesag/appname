...
import { Field, AllocationType } from '@/lib/types';
import { setAllocationTypeMembership } from '@/lib/actions';
import { fmt } from '@/lib/rules';

/**
 * Bulk-edit which fields use one allocation type. One type per field, so ticking
 * a field that's in another type MOVES it here (a small badge flags that). Mirrors
 * GroupMembershipEditor; commits the whole new set via setAllocationTypeMembership.
 */
export function TypeMembershipEditor({
  type, fields, types,
}: {
  type: AllocationType;
  fields: Field[];
  ...
  const initial = useMemo(
    () => new Set(fields.filter((f) => f.allocation_type_id === type.id).map((f) => f.id)),
    [fields, type.id],
  );
  ...
  const typeName = useMemo(() => new Map(types.map((t) => [t.id, t.label])), [types]);
  ...
  function save() {
    ...
    fd.set('allocation_type_id', type.id);
    fd.set('field_ids', Array.from(selected).join(','));
    startTransition(async () => {
      try { await setAllocationTypeMembership(fd); router.refresh(); }
  ...
  return (
    <div>
      <div style={{ fontSize: 12.5, color: 'var(--muted)', margin: '4px 0 12px', lineHeight: 1.5 }}>
        Tick the fields run as <strong style={{ color: 'var(--ink)' }}>{type.label}</strong>. A field can only have one
        type, so ticking one that's another type will move it here. Unticking leaves it untyped.
      </div>
      ...
      {sorted.map((f) => {
        const other = f.allocation_type_id && f.allocation_type_id !== type.id ? typeName.get(f.allocation_type_id) : null;
        return (
          <label key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 10px', marginBottom: 6, background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 6, cursor: 'pointer' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                {fmt(f.ha, 1)} ha{other && <span style={{ color: 'var(--amber, #b88a3a)', marginLeft: 8 }}>currently {other}</span>}
              </div>
            </div>
          </label>
        );

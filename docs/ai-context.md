...
From the Home dashboard the quick-access cards are: **P & K status**, **Fertiliser plan**, **Spread report**, **Field snapshot**, **Grazing**, **Lime status**, **Crop guide**, and **Farm map**. The header also has **Import a document** and **Add field** (both admin-only). Reports are reached from Home, the Grazing hub, or Activity — not from a tab of their own.
...
A new admin lands on a welcome screen to set the **unit system** (acres or hectares) and the **farm name**, after which the app opens. Staff who join via an invite code skip onboarding.
...
- **Season**: runs **1 September to 31 August**. The "2026 season" means Sep 2025 – Aug 2026. "This season" totals and report eligibility use this window.
...
- The assistant has **read-only** access to the user's farm data through a set of database tools. *(Tool names and exact parameters to be listed here once they are defined in the build.)*
